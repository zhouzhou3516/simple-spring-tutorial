...
public class UserDao {
    ...
    public long addUser(final UserInfo user) {
        final String INSERT_SQL = "INSERT INTO `user` (username, password,salt) values(?, ?,?)";
    ...
    public UserInfo selectUserById(long id) {
        try {
            return this.jdbcTemplate.queryForObject(
                    "select `id`, `username`, `password` ,`salt`  from `user` where `id` = ? ",
                    new Object[]{id},
                    new userMapper());
    ...
    public UserInfo selectUserByName(String name) {
        try {
            return this.jdbcTemplate.queryForObject(
                    "select `id`, `username`, `password`,`salt`  from `user` where `name` = ? ",
                    new Object[]{name},
                    new userMapper());
        } catch (EmptyResultDataAccessException e) {
            return null;
    ...
    public UserInfo selectUser(String username, String password) {
        try {
            return this.jdbcTemplate.queryForObject(
                    "select `id`, `username`, `password` ,`salt`  from `user` where `username` = ? AND `password` = ?",
    ...
    public List<UserInfo> userPage(int offset, int pageSize) {
        try {
            return this.jdbcTemplate.query(
                    "SELECT `id`, `username`, `password`,`salt`   FROM `user`  LIMIT ?,?",
