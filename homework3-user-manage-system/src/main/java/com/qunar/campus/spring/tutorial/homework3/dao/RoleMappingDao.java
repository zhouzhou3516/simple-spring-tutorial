...
public class RoleMappingDao {
    ...
    public RoleMapping getByUserId(long userId) {
        return this.jdbcTemplate
                .queryForObject("select id,user_id,role_id from role_mapping where user_id=?",
                        new Object[]{userId}, (rs, rowNum) -> {
                            RoleMapping roleMapping = new RoleMapping();
                            roleMapping.setId(rs.getInt("id"));
                            roleMapping.setUserId(rs.getInt("user_id"));
                            roleMapping.setRoleId(rs.getInt("role_id"));
                            return roleMapping;
                        });
    }
}
